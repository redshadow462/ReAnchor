...
def webauthn_login_verify():
    ...
    if not user_id or not challenge_b64:
        return jsonify({
            "error": "Passkey login session expired."
        }), 400
    ...
    try:
        ...
        for row in credentials:
            ...
            stored_credential_id = row[0]

            if stored_credential_id == credential_id:
                matching_credential = row
                break
        ...
        stored_credential_id = matching_credential[0]
        stored_public_key = matching_credential[1]
        stored_sign_count = matching_credential[2]
        ...
        return jsonify({
            "error":
                f"Passkey verification failed: {str(e)}"
        }), 401
